...
Future<bool> recordTransaction(String title, String msg, bool status, String idTrans, double amount) async {
  try {
...
void showSuccessDialog(BuildContext context,String title,String msg, double amount) {
    showDialog(
      context: context,
...
Future<void> generateAndSharePDF(Map<String, dynamic> details) async {
  ...
  final date = DateTime.tryParse(details['date'].toString()) ?? DateTime.now();
...
void showLogoutWarning() {
 ...
 Future<bool> balanceValidation({
  ...
  try {
    ...
    if (statusFeeIncluded == true) {
      double percentCharge = amount * 3 / 100;
      payable = amount - percentCharge;
    } else {
      double percentCharge = amount * 3 / 100;
      payable = amount + percentCharge;
